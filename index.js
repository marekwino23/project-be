...
app.get('/list', (req, res) => {
    db.query(`SELECT users.id, users.name, users.surname, users.email, data.date, data.hour, data.service FROM users JOIN data ON users.id = data.user_id `,function (err, result) {
        if(err) {
            console.log(err); 
            res.json({"error":true});
        }
